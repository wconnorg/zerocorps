...
| A leaked secret                                                      | Secrets only in `.env.local` and the host's settings; GitHub secret scanning and push protection; rotation runbook below; the coding assistant's file tools are denied every env file except the example (`.claude/settings.json`), so an open editor tab cannot put one in a transcript      | M2       |
...
| An encrypted, verified database link  | TLS is always on. `npm run db:check` showed on 2026-09-20 that Supabase's pooler presents a certificate signed by Supabase's own authority, which the public ones cannot vouch for. So today the link is encrypted but the server is not proven to be Supabase. Approved by the owner on 2026-09-21, not built yet: pin Supabase's published CA certificate before the milestone 2 release, failing closed. | Issue our own certificate and verify it (`verify-full`).                                                                                                  | M2    |
