 ...
 *
 * The functions are given the same Drizzle database the app gives them, so what is tested
 * is what runs, including the way Drizzle wraps the database's errors: "taken" is read out
 * of a wrapped unique violation, and that path is the one every clash below goes through.
 */
...
describe("claiming a name", () => {
  it("takes it, lower-cased, and leaves the first change free", async () => {
    ...
    const result = await setUsername(database.db, {
      userId: id,
      username: "  Trader_99  ",
      now: START,
    });
  ...
  it("re-typing the name you already have changes nothing and is not a change", async () => {
    const id = await newMember();
    await setUsername(database.db, { userId: id, username: "steady", now: START });
    const again = await setUsername(database.db, { userId: id, username: "STEADY", now: later(1) });
  ...
  it("refuses a name that breaks the rules, and writes nothing", async () => {
    ...
    for (const bad of ["ab", "has space", "admin", "___", "Trader!"]) {
      const result = await setUsername(database.db, { userId: id, username: bad, now: START });
  ...
  it("refuses a name somebody else has, whatever case it is typed in", async () => {
    ...
    await setUsername(database.db, { userId: first, username: "onlyone", now: START });
  ...
  it("the DATABASE is what stops two people having one name, not the check before it", async () => {
    const first = await newMember();
    const second = await newMember();
    await setUsername(database.db, { userId: first, username: "contested", now: START });
...
describe("changing a name", () => {
  it("holds the old name, so nobody can pick it up and be mistaken for you", async () => {
    ...
    await setUsername(database.db, { userId: mover, username: "oldname", now: START });
    const changed = await setUsername(database.db, {
      userId: mover,
      username: "newname",
      now: later(1),
    });
    ...
    const grab = await setUsername(database.db, {
      userId: stranger,
      username: "oldname",
      now: later(2),
    });
    ...
    expect(await isUsernameAvailable(database.db, "oldname", later(2))).toBe(false);
  });

  it("lets the old name go once the hold has run out", async () => {
    const mover = await newMember();
    const stranger = await newMember();
    await setUsername(database.db, { userId: mover, username: "passedon", now: START });
    await setUsername(database.db, { userId: mover, username: "movedaway", now: later(1) });

    const dayBefore = later(1 + HOLD_DAYS - 0.5);
    const dayAfter = later(1 + HOLD_DAYS + 0.5);
    expect(await isUsernameAvailable(database.db, "passedon", dayBefore)).toBe(false);
    expect(await isUsernameAvailable(database.db, "passedon", dayAfter)).toBe(true);
  ...
  it("makes you wait before the next change, and says until when", async () => {
    const id = await newMember();
    await setUsername(database.db, { userId: id, username: "first", now: START });
    // The first change is free.
    expect(
      await setUsername(database.db, { userId: id, username: "second", now: later(1) }),
    ).toMatchObject({ ok: true, changed: true });

    const tooSoon = await setUsername(database.db, {
      userId: id,
      username: "third",
      now: later(2),
    });
    ...
    // The settings page is told the same date, and told nothing once it has passed.
    expect((await nextUsernameChangeAt(database.db, id, later(2)))?.getTime()).toBe(
      later(1 + COOLDOWN_DAYS).getTime(),
    );

    // And the wait really does end.
    const afterTheWait = later(1 + COOLDOWN_DAYS + 0.5);
    expect(await nextUsernameChangeAt(database.db, id, afterTheWait)).toBeNull();
    expect(
      await setUsername(database.db, { userId: id, username: "third", now: afterTheWait }),
    ).toMatchObject({ ok: true, username: "third" });
  });

  it("the wait cannot be dodged by asking twice at once", async () => {
    const id = await newMember();
    await setUsername(database.db, { userId: id, username: "once", now: START });
    await setUsername(database.db, { userId: id, username: "twice", now: later(1) });

    // Both arrive while the wait is on. Each reads the row itself, so both are refused.
    const [a, b] = await Promise.all([
      setUsername(database.db, { userId: id, username: "aaa", now: later(2) }),
      setUsername(database.db, { userId: id, username: "bbb", now: later(2) }),
    ]);
  ...
  it("your own name and your own held name do not block you", async () => {
    const id = await newMember();
    await setUsername(database.db, { userId: id, username: "mine", now: START });
    expect(await isUsernameAvailable(database.db, "mine", START, id)).toBe(true);
    await setUsername(database.db, { userId: id, username: "myother", now: later(1) });
    // "mine" is held BY this member, so it is theirs to take back when the wait ends.
    expect(await isUsernameAvailable(database.db, "mine", later(2), id)).toBe(true);
    expect(await isUsernameAvailable(database.db, "mine", later(2))).toBe(false);
  });
});
...
describe("letting go of names that are no longer held", () => {
  ...
  it("a hold that has run out is free whether or not anything has cleared it", async () => {
    const id = await newMember();
    await setUsername(database.db, { userId: id, username: "uncleared", now: START });
    await setUsername(database.db, { userId: id, username: "current", now: later(1) });
    // Nothing tidies it, and it is still free once the hold has run out.
    expect(await isUsernameAvailable(database.db, "uncleared", later(1 + HOLD_DAYS + 1))).toBe(
      true,
    );
